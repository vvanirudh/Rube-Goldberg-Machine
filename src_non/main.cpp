...
int main(int argc, char* argv[])
{
  ...
  for (int i = 1; i <= atoi(argv[1]); i++) {
	  my_world->Step(time_step, settings.velocity_iterations, settings.position_iterations);
	  const b2Profile& p = my_world->GetProfile();
	  sum += p.step;
	  vsum+=p.solveVelocity;
	  psum+=p.solvePosition;
	  csum+=p.collide;
  }
  gettimeofday(&time,NULL);
  ...
  //std::cout<<int(t2)<<"\n";
  double average = (sum/atoi(argv[1]));
  vsum=(vsum/atoi(argv[1]));
  csum=(csum/atoi(argv[1]));
  psum=(psum/atoi(argv[1]));
  
  std::cout<<"Total Iterations: "<< atoi(argv[1])<<"\n";
  std::cout<<"Average time per step is "<< average<<" ms\n";
  std::cout<<"Average time for collisions is "<<csum<<" ms\n";
  std::cout<<"Average time for velocity updates is "<<vsum<<" ms\n";
  std::cout<<"Average time for position updates is "<<psum<<" ms\n";
  std::cout<<"Total time for loop is "<<(t2-t1)*1000<<" ms\n";
